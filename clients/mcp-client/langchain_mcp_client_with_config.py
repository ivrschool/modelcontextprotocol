...
This file implements a LangChain MCP client that:
  - Loads configuration from a JSON file specified by MCP_SERVER_CONFIG environment variable.
...
Detailed explanations:
  ...
  - Environment Variable: MCP_SERVER_CONFIG should point to a config JSON that defines all MCP servers.
"""
...
def read_config_json():
    ...
    Priority:
      1. Try to read the path from the MCP_SERVER_CONFIG environment variable.
      2. If not set, fallback to a default file 'mcp_server_config.json' in the same directory.
    ...
    if not config_path:
        ...
        config_path = os.path.join(script_dir, "mcp_server_config.json")
